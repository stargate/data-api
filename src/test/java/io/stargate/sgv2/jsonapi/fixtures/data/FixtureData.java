 ...
 * CqlFixture}.
 *
 * <p>See the {@link DefaultFixtureDataSupplier}, the idea is that we can generate test data that tests min / max
...
public interface FixtureData {
   ...
   */
  List<FixtureData> SUPPORTED = List.of(
      new DefaultData(),
      new MaxNumericData(),
      new MinNumericData(),
      new AllNullValues()
   ...
   */
  List<FixtureData> OUT_OF_RANGE = List.of(
      new OverflowData()
  );
   ...
   */
  List<FixtureData> UNSUPPORTED_TYPES = List.of(
      new UnsupportedTypesData()
  );

...
public class CommandResultAssert extends AbstractAssert<CommandResultAssert, CommandResult> {
    ...
    public CommandResultAssert isDDLSuccess(){
        ...
        assertThat(actual.data())
                .as("isDDLSuccess() - data is null")
                .isNull();
        assertThat(actual.status())
                .as("isError() - status is no empty")
                .isNotEmpty();
        assertThat(actual.errors())
                .as("isDDLSuccess() - errors is empty")
                .isEmpty();
        return this;
    }
    ...
    public CommandResultAssert isError(){
        ...
        assertThat(actual.data())
                .as("isError() - data is null")
                .isNull();
        assertThat(actual.status())
                .as("isError() - status is empty")
                .isEmpty();
        assertThat(actual.errors())
                .as("isError() - errors is not null")
                .isNotNull();
        assertThat(actual.errors())
                .as("isError() - errors is not empty")
                .isNotEmpty();
        return this;
    }
    ...
    public CommandResultAssert hasOnlyStatus(CommandStatus key, Object value) {
        assertThat(actual.status())
                .as("status is not null")
                .isNotNull();
        assertThat(actual.status())
                .as("status is only one")
                .hasSize(1);
        assertThat(actual.status().get(key))
                .as("status['%s']", key)
                .isEqualTo(value);

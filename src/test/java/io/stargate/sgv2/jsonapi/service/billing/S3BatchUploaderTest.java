...
class S3BatchUploaderTest {
  ...
  //          List.of(LINE_A, LINE_B), Instant.parse("2026-05-20T14:23:11.482Z"));
  //
  ...
  //    S3AsyncClient client = mock(S3AsyncClient.class);
  //    when(client.putObject(any(PutObjectRequest.class), any(AsyncRequestBody.class)))
  //        .thenReturn(CompletableFuture.failedFuture(new RuntimeException("simulated S3
  // failure")));
  //
  //    var uploader = uploader(client);
  ...
  //
  //    // retries and per-call timeouts live in the client configuration, so exactly one putObject
  // per

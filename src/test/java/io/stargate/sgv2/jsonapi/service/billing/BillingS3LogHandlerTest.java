...
class BillingS3LogHandlerTest {
  ...
  //
  //  /**
  //   * First-ever Uni creation in a JVM registers the SmallRye context-propagation provider
  // through
  //   * {@code ContextManagerProvider.instance()}, whose ServiceLoader loop both CAS-races
  // concurrent
  //   * callers and throws "ContextManagerProvider already set" when it discovers a second provider
  // —
  //   * possibly after having registered the first. Racing that from concurrent producer threads
  // makes
  ...
  //      // ContextManagerProvider.INSTANCE is now set and concurrent callers can no longer race
  // it.
  //    }
  //    Uni.createFrom()
  //        .item(0)
  ...
  //
  //  /**
  //   * Programmable {@link AsyncBatchedLogUploader}: records every batch and settles the returned
  // Uni
  //   * per {@link Mode}. Never blocks a caller thread — HOLD parks the completion in {@code held}
  // for
  //   * the test to release explicitly.
  //   */
  ...
  //        case FAIL -> future.completeExceptionally(new RuntimeException("simulated upload
  // failure"));
  ...
  //    try {
  //      // Enqueue order is publish order for a single producer; event-time order is not (the
  // second
  ...
  //      // Asserts the condition holds for the whole window — i.e. that an async flush did NOT
  // happen
  //      await()
  ...
  //    var uploader = new RecordingUploader();
  //    var registry = new SimpleMeterRegistry();
  //    // Raw constructor: newHandler() pins maxAge to NEVER; this is the one test that wants a
  // live
  //    // tick.
  //    var handler =
  ...
  //      // Wait for the single slot to be claimed and its 1-line batch drained: from here the
  // queue
  ...
  // assertThat(uploader.inFlight.get()).isEqualTo(2));
  //
  ...
  //      // First await = arrival: the one upload has started. Second = the gate holds: batches
  // stays
  //      // at exactly one for 200ms, proving concurrency=1 keeps the other two sealed batches
  // queued.
  //      await().atMost(AWAIT).untilAsserted(() -> assertThat(uploader.batches).hasSize(1));
  //      await()
  //          .during(Duration.ofMillis(200))
  ...
  //
  //  /**
  //   * {@code close()} runs concurrently with in-flight {@code publish()} calls in production — a
  // pod
  //   * shutdown doesn't wait for request threads to go quiet first. This drives both at once: 4
  //   * threads publish flat out while the main thread calls {@code close()} mid-stream, then keeps
  // the
  ...
  //   * returns instead of hanging, delivered lines are a duplicate-free subset of what was
  // published,
  //   * and the metrics reconcile as {@code flushed + dropped <= offered} rather than {@code ==}.
  // The
  //   * gap is expected, not a bug: a publish can land after close() takes its final buffer
  // snapshot,
  //   * so that line is neither delivered nor counted as dropped — see {@link
  //   * BatchedLogUploaderMetrics}'s class doc for this same at-most-once slippage. The log line
  // below
  //   * reports the exact gap each run.
  //   */
  //  @Test
  //  void closeRacingProducersNeverHangsAndReconciles() throws Exception {
  //    var uploader = new RecordingUploader();
  //    var registry = new SimpleMeterRegistry();
  //    var handler = newHandler(uploader, registry, 5, 1_000_000_000L, 1000, 4,
  // Duration.ofSeconds(1));
  //
  //    int threads = 4;
  //    // Producers normally exit on the stop flag below. The cap bounds the sad path (a hung close
  //    // never reaches stop.set) so no producer spins forever — executor.shutdown() does not
  // interrupt
  ...
  //    }
  //
  //    // Land close() deterministically amid in-flight publishes: wait until producers have
  // flooded
  //    // the pipeline (2x the 1000-slot buffer → buffer full, overflow dropping, uploads gated),
  // not a
  ...
  //    executor.shutdown();
  //
  //    // publish must never throw, close must return. Post-close publishes can settle after
  // close()'s

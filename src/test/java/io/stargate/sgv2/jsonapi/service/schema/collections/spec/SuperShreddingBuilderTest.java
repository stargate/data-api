 ...
 * <p>
 * The testing process is a little complicated, but here are the reasons: we want to avoid repeating the
 * table def in many places, avoid testing raw CQL, we want to be able to test high-level things
 * list the statements or the schema of a table actually in the DB. And finally, once you get to a
 * real table, it is "bound" with names and real values, which often results in using the same name for all
 * tables etc. because it makes the test easier. The end result we want is that we can test bound
 * metadata and statements that represent a real named table with values without needing hard coded
 * CQL, and without repeated code that builds cql.
 * </p>
 * <p>
 *  So the testing strategy is below, building up on each layer:
 *  <ol>
 *      <li>A single instance of CQL of a specific example of super shredding is defined in the
 *      test class {@link SuperShreddingCQLBuilderTest}, which validates that {@link SuperShreddingCQLBuilder}
 *      can create CQL that matches this specific example. This is our base level ground truth.</li>
 *      <li>Creating TableMetadata and IndexMetadata objects via the {@link SuperShreddingMetadataBuilder} it tested
 *      by the test class {@link SuperShreddingMetadataBuilderTest} which validates the CQL generated by the driver
 *      for these objects using the {@link SuperShreddingCQLBuilder}. TableMetadata is "bound" with names etc,
 *      and we use it as test data to represent what the driver returns about a table.
 *      </li>
 *      <li>TODO: we generate SimpleStatements via a build, and validate the CQL agains the cql builder</li>
 *  </ol>
 *  More simply, dobelow with minimum duplication:
 *  <ol>
 *      <li>Validate dynamic cql string against static cql string.</li>
 *      <li>Validate faked driver metadata against previously validated dynamic cql string</li>
 *      <li>Validate super shredding table predicate againdt previously validated metadata</li>
 *      <li>Validate statement objects against previously validated dynamic cql string</li>
 *  </ol>
 * </p>
 */

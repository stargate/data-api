...
public class CqlSessionIntegrationTest {
  ...
  public class CqlSessionCacheTimingTest {
    ...
    public void testOSSCxCQLSessionCacheTimedEviction()
      ...
      // use refection to set a lower ttl, so that we don't have to create another config override
      // and profile
      Field sessionCacheTtlSeconds =
          operationsConfig.databaseConfig().getClass().getDeclaredField("sessionCacheTtlSeconds");
      sessionCacheTtlSeconds.setAccessible(true);
      sessionCacheTtlSeconds.set(operationsConfig.databaseConfig(), 10);
      ...
      sessionCacheTtlSeconds.set(operationsConfig.databaseConfig(), 300);
    }
  }
  ...
  public class InvalidCredentialsTest {
    ...
    public void testOSSCxCQLSessionCacheWithInvalidCredentials()
        throws NoSuchFieldException, IllegalAccessException {
      // use refection to set an invalid-password, so we don't have to create another config
      // override and profile
      Field cassandraPassword =
          operationsConfig.databaseConfig().getClass().getDeclaredField("password");
      cassandraPassword.setAccessible(true);
      cassandraPassword.set(operationsConfig.databaseConfig(), "invalid-password");
      ...
      cassandraPassword.set(operationsConfig.databaseConfig(), "cassandra");
    }
  }
}

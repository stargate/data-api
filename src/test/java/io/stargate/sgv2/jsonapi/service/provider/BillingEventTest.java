...
class BillingEventTest {
  ...
  private ModelUsage createModelUsage(ModelProvider provider, ModelType modelType, int totalTokens) {
    Tenant tenant = Tenant.create(DatabaseType.ASTRA, "53950f2d-4d4c-4346-a84e-7a07e2ab23f4");
    return new ModelUsage(
        provider, modelType, "test-model", tenant, ModelInputType.INDEX, 100, totalTokens, 0, 0,
        1000L);
  }
  ...
  void fromModelUsage_nvidiaEmbedding() throws Exception {
    ...
    BillingEvent event = BillingEvent.from(usage, "serverless", "serverless_database");

    assertThat(UUID.fromString(event.id())).isNotNull();
    assertThat(Instant.parse(event.timestamp())).isNotNull();
    assertThat(event.product()).isEqualTo("serverless");
    ...
    assertThat(event.properties().resourceType()).isEqualTo("serverless_database");
    assertThat(event.properties().resourceId())
        .isEqualTo("53950f2d-4d4c-4346-a84e-7a07e2ab23f4");
  }
  ...
  void jsonSerialization_matchesBillingSchema() throws Exception {
    ...
    assertThat(props.get("resource_id").asText())
        .isEqualTo("53950f2d-4d4c-4346-a84e-7a07e2ab23f4");
  }

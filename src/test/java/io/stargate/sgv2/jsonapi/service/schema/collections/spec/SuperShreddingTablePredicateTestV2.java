...
public class SuperShreddingTablePredicateTestV2 extends SuperShreddingBuilderTest{
    private static final Logger LOGGER = LoggerFactory.getLogger(SuperShreddingTablePredicateTestV2.class);


    private void assertPredicate(String testName,  boolean expectedResult, SuperShreddingPredicateBuilder predicateBuilder,  SuperShreddingMetadataBuilder builder, String logMessage){
        assertPredicate(testName, expectedResult,predicateBuilder.buildTableOnly(),  (TableMetadata) builder.buildTableOnly(), logMessage);
    }

    private void assertPredicate(String testName,  boolean expectedResult, SuperShreddingTablePredicate predicate,  TableMetadata tableMetadata, String logMessage) {

       try (var logWrapper = new LoggerTestWrapper(SuperShreddingTablePredicate.class)) {

           if (LOGGER.isInfoEnabled()) {
               LOGGER.info("{} - expectedResult:{} , tableMetadata:{}", testName, expectedResult, tableMetadata == null ? "null" :  tableMetadata.describe(true));
           }

           var predicateResult = predicate.test(tableMetadata);
           LOGGER.info("{} - expectedResult:{}, predicateResult:{}", testName, expectedResult, predicateResult);
    ...
    public void removeColumns() {
        ...
        removeAllColumns(tableMetadata, SuperShreddingMetadata.Identifiers.ALL).forEach(entry -> {
            assertPredicate(
    ...
    public void removePartitionKey() {
        ...
        removeAllPartitionKeys(tableMetadata).forEach(entry -> {
            assertPredicate(
                    "removePartitionKey(%s)".formatted(entry.column()),
                    false,
                    predicate,
                    entry.tableMetadata(),
                    "partition key missing, columns: "+ cqlIdentifierToMessageString(entry.column()));
        });
    ...
    public void swapColumnTypes() {
        ...
        // we expect all columns to be present, so use that as the list
        swapTypesAllColumns(tableMetadata, SuperShreddingMetadata.Identifiers.ALL, DataTypes.TINYINT, DataTypes.TEXT).forEach(entry -> {
            assertPredicate(
    ...
    public void unexpectedPartitionKeys() {
        ...
        assertPredicate(
                ...
                "unexpected columns in partition key, columns: %s(%s)".formatted(columnName, errFmt(DataTypes.TEXT)));
    ...
    public void unexpectedClusteringColumns() {
        ...
        assertPredicate(
                ...
                predicate,
                updatedTable,
                "unexpected columns in clustering key, columns: %s(%s)".formatted(columnName, errFmt(DataTypes.TEXT)));
    }
    ...
    public void unexpectedColumnsStrictMode() {
        ...
        assertPredicate(
                ...
                predicate,
                updatedTable,
                "unexpected columns in strict mode, columns: unexpected_column(text)".formatted(columnName, errFmt(DataTypes.TEXT)));
    }
    ...
    public void unexpectedColumnsRelaxedMode() {
        ...
        var predicate = configAllOptional(SuperShreddingBuilder.predicate())
                .withStrict(false)
                .buildTableOnly();

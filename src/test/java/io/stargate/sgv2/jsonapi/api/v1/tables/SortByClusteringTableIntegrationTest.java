...
public class SortByClusteringTableIntegrationTest extends AbstractTableIntegrationTestBase {
  ...
  public void emptyFilterTriggersImMemorySort() {
    ...
    assertTableCommand(keyspaceName, TABLE_NAME)
        .templated()
        .find(new HashMap<>(), PROJECT_OFFSET, sort, ImmutableMap.of("limit", 1))
        .wasSuccessful()
        .hasWarning(
            0,
  ...
  @Test
  public void invalidPartitionColumnFilterTriggersImMemorySort() {
    Map<String, Object> sort =
        ImmutableMap.of(fieldName(ThreeClusteringKeysTableScenario.CLUSTER_COL_1), -1);
    var expected =
        """
            ...
            """;
    Map<String, Object> invalidFilterOnPartitionKey =
        ImmutableMap.of(
            fieldName(ThreeClusteringKeysTableScenario.ID_COL), ImmutableMap.of("$ne", "invalid"));
    assertTableCommand(keyspaceName, TABLE_NAME)
        .templated()
        .find(invalidFilterOnPartitionKey, PROJECT_OFFSET, sort, ImmutableMap.of("limit", 1))
        .wasSuccessful()
        .hasWarning(
            0,
            WarningException.Code.IN_MEMORY_SORTING_DUE_TO_PARTITION_KEY_NOT_RESTRICTED,
        ...
        .hasWarning(
            1,
            // Since ID_COL has no SAI index on it, need allowFiltering to perform $ne
            WarningException.Code.QUERY_RETRIED_DUE_TO_INDEXING,
            "detect that the query generated by the command was inefficient due to a lack of primary key or index usage")
        .hasDocuments(1)
        .hasDocumentInPosition(0, expected);
  }
}

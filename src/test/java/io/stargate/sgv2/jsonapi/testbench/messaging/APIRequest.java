 ...
 *
 * <p>Handles retries based on detecting substrings in the response body, these occur before {@link
 * #execute()} returns.
 */
public class APIRequest {
   ...
   * @param testRunEnv Environment for this test run, used to find schema names to use in the URL
   *     path.
   * @param request the complete API request to send, <b>NOTE:</b> any substitutions into the body
   *     of the request must also be done.
   */
   ...
   *
   * <p>No validation of the response is performed, that is left for assertions to handle later.
   *
  ...
  private RequestSpecification requestSpec() {
    ...
    return requestForTaget().headers(getHeaders()).body(requestString).when();
  }
  ...
  protected Map<String, String> getHeaders() {
    ...
    headers.put(
        ...
        testRunEnv.requiredValue(HttpConstants.AUTHENTICATION_TOKEN_HEADER_NAME));
  ...
  private Response executeRequest(RequestSpecification requestSpec) {
    ...
    if (commandName.getTargets().contains(CommandTarget.COLLECTION)
      ...
      rawRresponse =
          requestSpec.post(KEYSPACE_PATH, testRunEnv.requiredValue(TestRunEnv.ENV_KEYSPACE_NAME));

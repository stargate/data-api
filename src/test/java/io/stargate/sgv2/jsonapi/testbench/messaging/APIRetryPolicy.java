...
interface APIRetryPolicy {
    ...
    /**
     * Retries are driven by the presence of the following keys in the {@link TestRunEnv}. When a retry is
     * performed the response for that request is lost, the response of the call to <code>execute</code> will
     * be the response from the last attempt.
     * <ul>
     *     <li>{@link #ENV_RETRY_MATCH_STRING} Enables retry if present and non blank string, treated as a tab
     *     {@link #RETRY_MATCH_STRING_DELIM} delimtered string. If any of the tokens is present in the response body
     *     the request is retried.
     *     Example: <code>"EMBEDDING_PROVIDER_RATE_LIMITED\tEMBEDDING_PROVIDER_TIMEOUT"</code></li>
     *    <li>{@link #ENV_RETRY_MAX_ATTEMPTS} total number of attempts to make before returning,
     *    must be above 1, default is {@link #ENV_RETRY_MAX_ATTEMPTS}</li>
     *    <li>{@link #ENV_RETRY_BASE_SLEEP_MS} base number of milliseconds between attempts, this is multiplied by
     *    2 ^ attempt, so base of 5000 means sleep of 5, 10, 20 seconds</li>
     *    <li>{@link #ENV_RETRY_JITTER_MS} Upper bound on the random number of milliseconds to add to each attempt.</li>
     * </ul>
     */
    static APIRetryPolicy createRetryPolicy(TestRunEnv testRunEnv) {
        ...
        var maxAttempts = Integer.parseInt(testRunEnv.get(ENV_RETRY_MAX_ATTEMPTS, DEFAULT_MAX_ATTEMPTS));
        var baseSleepMs = Long.parseLong(testRunEnv.get(ENV_RETRY_BASE_SLEEP_MS, DEFAULT_BASE_SLEEP_MS));
        var jitterMs = Long.parseLong(testRunEnv.get(ENV_RETRY_JITTER_MS, DEFAULT_JITTER_MS));

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("createRetryPolicy() - retryMatch={}, maxAttempts={}, baseSleepMs={}, jitterMs={}", retryMatch, maxAttempts, baseSleepMs, jitterMs);
        }

        return new ConfiguredAPIRetryPolicy(List.of(retryMatch.split(RETRY_MATCH_STRING_DELIM)), maxAttempts, baseSleepMs, jitterMs);
    }
     ...
     * @return A decision to retry or not, along with the attempt count, use for the next call to this method.
     */
    RetryDecision decide(RetryDecision lastDecision, Response response);


    /**
     * A decision to retry or not, along with the attempt count. This is where we count the
     * number of attempts.
     */
    record RetryDecision(boolean retry,
                                 int attempt){
    ...
    /**
     * Configurable retry policy with customizable retry conditions, maximum attempts, and backoff strategy.
     */
    record ConfiguredAPIRetryPolicy(
            List<String> retryMatch,
            int maxAttempts,
            long baseSleepMs,
            long jitterMs
        ...
        public ConfiguredAPIRetryPolicy {
            ...
            if (maxAttempts < 2) {
                throw new IllegalArgumentException("maxAttempts must be greater than 1, got: " + maxAttempts);
            }
        }
        ...
        public RetryDecision decide(RetryDecision lastDecision, Response response) {
            ...
            for (var match : retryMatch) {
                if (body.contains(match)) {
                    ...
                    LOGGER.info("executeRequest() - Retrying, found retry string in response. match={}, sleepMs={} ms, lastDecision.attempt={}", match, sleepMs, lastDecision.attempt);
                    try {

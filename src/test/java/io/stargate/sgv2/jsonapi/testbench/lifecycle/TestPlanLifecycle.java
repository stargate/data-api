 ...
 * <p>
 * Design to be implemented by a {@link io.stargate.sgv2.jsonapi.testbench.targets.Backend} so that it can
 * make changes to the data environement so tests can run in a common environement. For example,
 * when we use Cassandra as a backend we need to create a keyspace but for Astra we use the default one.
 * </p>
 * <p>
 * There should not be any test logic within the implementations, that should all be in the test defintoions.
 * </p>
 * <p>
 * Extracted to an interface so it can be reused, such as at the higher level
 * {@link io.stargate.sgv2.jsonapi.testbench.targets.Target} which includes a backend.
 * </p>
 * <p>
 * All methods allow the implementation to return a JUNIT {@link DynamicNode} which will be inserted before or after
 * the dynamic nodes at that level. The returned node could be a single {@link org.junit.jupiter.api.DynamicTest} or
 * a {@link org.junit.jupiter.api.DynamicContainer}.
 * </p>
 */
public interface TestPlanLifecycle {
   ...
   * @param env The runtime environment for this test, including resolved variables and configuration.

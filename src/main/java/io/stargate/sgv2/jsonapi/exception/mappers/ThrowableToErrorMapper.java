...
/**
 * amorton - 7 jan 2026 refactor
 * NOTES: leaving this named ThrowableToErrorMapper because we know it as that name.
 *
 * Changed the role to ONLY be remaping Java exception to exception. Previously it would also build the
 * Command error (CommandResult.Error) that is returned to the client. Made the change to try
 * simplifying as I work to remove this class completly.
 */
public final class ThrowableToErrorMapper {
  ...
  public static Throwable mapThrowable(Throwable throwable, String message) {
    ...
    // amorton - 7 jan 2026 - this could be better as pattern matching, but leaving until we remove the whole class.
...
//      return jae.getCommandResultError(message, jae.getHttpStatus());
    ...
    if (throwable instanceof UnauthorizedException) {
      return SecurityException.Code.UNAUTHENTICATED_REQUEST
          .get();
...
//  private static APIException handleQueryExecutionException(QueryExecutionException throwable, String message) {
//
...
//  private static APIException handleQueryValidationException(QueryValidationException throwable, String message) {
...
//  private static CommandResult.Error handleAllNodesFailedException(AllNodesFailedException throwable, String message) {
//
...
//  }
  ...
  private static APIException handleJsonProcessingException(
...
//          .getCommandResultError(Response.Status.BAD_REQUEST);
  ...
  private static APIException handleUnrecognizedException(Throwable throwable, String message) {
    ...
    return ServerException.Code.UNEXPECTED_SERVER_ERROR
        .get(ErrorFormatters.errVars(throwable));
  }
}

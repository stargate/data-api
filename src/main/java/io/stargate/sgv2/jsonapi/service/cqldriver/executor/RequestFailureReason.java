...
/**
 * Roughly copied from `RequestFailureReason` in the Cassandra code base, but simplified to only the codes.
 * <p>
 * <b>NOTE:</b> The codes here need to be in sync with the C* code, the exceptions in the drivers only use the codes.
 * Sync with <em>Astra</em> version of the code not the DataStax because the Astra version has more codes.
 * <p>
 * Used with {@link com.datastax.oss.driver.api.core.servererrors.ReadFailureException} and similar errors.
 */

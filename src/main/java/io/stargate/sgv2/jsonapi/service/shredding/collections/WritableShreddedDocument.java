...
public record WritableShreddedDocument(
   ...
   * {@link DocumentShredderListener} for automated construction when traversing a Document.
   */
  public static class Builder implements DocumentShredderListener {
    /**
    ...
    public void shredVector(JsonPath path, String binaryVector) {
      ...
      byte[] binaryPayload;
      try {
        binaryPayload = Base64.getDecoder().decode(binaryVector);

...
public class WriteableTableRowBuilder {
  ...
  public static final CqlNamedValue.ErrorStrategy<DocumentException> ERROR_STRATEGY = new CqlNamedValue.ErrorStrategy<>() {
    @Override
  ...
  public WriteableTableRowBuilder(
    ...
    this.tableSchemaObject = Objects.requireNonNull(tableSchemaObject, "schemaObject cannot be null");
  ...
  public WriteableTableRow build(JsonNamedValueContainer source) {
    ...
    var allColumns = new CqlNamedValueFactory(tableSchemaObject, codecRegistry, ERROR_STRATEGY).create(source);
...
//  }
  ...
  /**
   * Checks if the row has all the primary key columns that are part of the table, and the values are not null
   *
   * <b>NOTE:</b> must check for codec errors before this, so we know we have a valid value to check
   *
  ...
  private void checkAllPrimaryKeys(CqlNamedValueContainer allColumns) {
    ...
    Predicate<ColumnMetadata> isMissing = columMetadata -> {
...
//            .filter(cqlNamedValue -> cqlNamedValue.errorCode().equals(INSERT_ERROR_CODES.unknownColumn()))
...
//            .filter(cqlNamedValue -> cqlNamedValue.errorCode().equals(ERROR_STRATEGY.missingCodec()))
//            .map(CqlNamedValue::name)
//            .sorted(CqlIdentifierUtil.CQL_IDENTIFIER_COMPARATOR)
//            .toList();
//
...
//        allColumns.values().stream()
//            .filter(cqlNamedValue -> cqlNamedValue.state().equals(NamedValue.NamedValueState.PREPARE_ERROR))
//            .filter(cqlNamedValue -> cqlNamedValue.errorCode().equals(ERROR_STRATEGY.codecError()))
//            .map(CqlNamedValue::name)
//            .sorted(CqlIdentifierUtil.CQL_IDENTIFIER_COMPARATOR)
//            .toList();
//

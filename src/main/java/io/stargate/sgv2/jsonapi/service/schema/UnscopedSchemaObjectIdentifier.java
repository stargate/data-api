...
/**
 * An identifier for a schema object that is not scoped to a {@link
 * io.stargate.sgv2.jsonapi.api.request.tenant.Tenant}.
 *
 * <p>We only have the Keyspace and object name, such as the table name, we do now know who it
 * belongs to. This is used by the SchemaObjectCache due to issues with not knowing if a CQL table
 * is a API Collection or API Table. See {@link SchemaObjectCache#getTableBased(RequestContext,
 * UnscopedSchemaObjectIdentifier, UserAgent, boolean)}
 */

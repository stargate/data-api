...
import com.datastax.oss.driver.api.core.metadata.schema.ColumnMetadata;import com.datastax.oss.driver.api.core.metadata.schema.IndexKind;import com.datastax.oss.driver.api.core.metadata.schema.IndexMetadata;import com.datastax.oss.driver.api.core.type.DataType;
import com.datastax.oss.driver.api.core.type.DataTypes;
import com.datastax.oss.driver.api.querybuilder.SchemaBuilder;
import com.datastax.oss.driver.api.querybuilder.schema.CreateTable;
import com.datastax.oss.driver.internal.core.metadata.schema.DefaultColumnMetadata;import com.datastax.oss.driver.internal.core.metadata.schema.DefaultIndexMetadata;import com.datastax.oss.driver.internal.querybuilder.schema.DefaultCreateIndex;
import io.stargate.sgv2.jsonapi.config.constants.TableDescConstants;import io.stargate.sgv2.jsonapi.config.constants.VectorConstants;import io.stargate.sgv2.jsonapi.service.cqldriver.override.ExtendedCreateIndex;
import io.stargate.sgv2.jsonapi.service.cqldriver.override.ExtendedVectorType;
import io.stargate.sgv2.jsonapi.service.schema.tables.ApiIndexFunction;import io.stargate.sgv2.jsonapi.service.schema.tables.CQLSAIIndex;
...
public interface SuperShreddingMetadata {
    ...
    /**
     * String names of all columns, in the order that we traditionally have them in the collection table,
     * pls try to keep the order :)
     */
    interface Names {
        ...
        List<String> ALL = List.of(
                ...
                QUERY_BOOLEAN_VALUES, QUERY_DOUBLE_VALUES, QUERY_NULL_VALUES,
                QUERY_TEXT_VALUES, QUERY_TIMESTAMP_VALUES,
    ...
    interface Identifiers {
        ...
        List<CqlIdentifier> ALL = List.of(
                KEY, TX_ID, DOC_JSON,
                EXIST_KEYS, ARRAY_SIZE, ARRAY_CONTAINS,
                QUERY_BOOLEAN_VALUES, QUERY_DOUBLE_VALUES, QUERY_NULL_VALUES,
                QUERY_TEXT_VALUES, QUERY_TIMESTAMP_VALUES,
    ...
    record ColumnDef(CqlIdentifier name, DataType type, ColumnMetadataFactory metadataFactory) {
        ...
        public ColumnMetadataPredicate predicate() {
            return new ColumnMetadataPredicate.BasicType(name, type);
        }
    }
    ...
    interface ColumnDefs {
        ...
        List<ColumnDef> ALL = List.of(
                KEY, TX_ID, DOC_JSON,
                EXIST_KEYS, ARRAY_SIZE, ARRAY_CONTAINS,
                QUERY_BOOLEAN_VALUES, QUERY_DOUBLE_VALUES, QUERY_TEXT_VALUES,
    ...
    interface Predicates {
        ...
        List<ColumnMetadataPredicate> ALL = List.of(
                KEY, TX_ID, DOC_JSON,
                EXIST_KEYS, ARRAY_SIZE, ARRAY_CONTAINS,
                QUERY_BOOLEAN_VALUES, QUERY_DOUBLE_VALUES, QUERY_TEXT_VALUES,
                QUERY_TIMESTAMP_VALUES, QUERY_NULL_VALUES,
        ...
        static List<ColumnMetadata> allUnexpectedColumns(List<ColumnMetadataPredicate> predicates, Collection<ColumnMetadata> columns) {
     ...
     *
     * <p>
     * In the `system_schema.indexes` the <code>options</code> field has the extra class_name and target. But in CQL
     * these are not in the <code>WITH OPTIONS</code>
     * </p>
     * <p>
     * Example of <code>system_schema.indexes</code>:
     * <pre>
    ...
    record IndexDef(ColumnDef columnDef, ApiIndexFunction indexFunction) {
        ...
        public CqlIdentifier indexName(CqlIdentifier collection) {
            return CqlIdentifier.fromInternal(collection.asInternal() + "_" + columnDef.name().asInternal());
        }
        ...
        public SimpleStatement statement(
            ...
            var start = SchemaBuilder.createIndex(indexName(collection)).custom(CQLSAIIndex.SAI_CLASS_NAME);
            if (ifNotExists) {
    ...
    interface IndexDefs {
        ...
        static List<IndexMetadata> toIndexMetadata(CqlIdentifier keyspace,
                                                   CqlIdentifier table,
                                                   List<IndexDef> indexes,
                                                   Map<IndexDef, Map<String, String>> perIndexOptions){

            Map<IndexDef, Map<String, String>> safeIndexOptions = perIndexOptions == null ? Collections.emptyMap() : perIndexOptions;

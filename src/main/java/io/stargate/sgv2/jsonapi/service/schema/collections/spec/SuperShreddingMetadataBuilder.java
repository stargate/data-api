...
public class SuperShreddingMetadataBuilder extends SuperShreddingBuilder<Describable, SuperShreddingMetadataBuilder> {
    ...
    public List<SuperShreddingComponent<Describable>> buildInternal() {
        ...
        // Primary key first
        var primaryKey = ColumnDefs.toColumnMetadata(superShreddingDef.keyspace(), superShreddingDef.collection(), ColumnDefs.PARTITION_KEY)
                .toList();
        ...
        // non primary key
        var columnDefs = superShreddingDef.hasAnyOptional() ?
        ...
        if (superShreddingDef.isVectorDefined()) {
            ...
            perColumnOptions.put(ColumnDefs.QUERY_VECTOR_VALUE, Map.of("dimensions", superShreddingDef.vectorLength()));
        ...
        buildIndexMetadata()
                .forEach(metadata -> indexMetadata.put(metadata.getName(), metadata));
        ...
        components.add(new SuperShreddingComponent<>(superShreddingDef.collection(), SuperShreddingComponentType.TABLE, tableMetadata));
    ...
    private Stream<IndexMetadata> buildIndexMetadata(){
        ...
        return SuperShreddingMetadata.IndexDefs.toIndexMetadata(superShreddingDef.keyspace(), superShreddingDef.collection(), defsAndOptions.indexDefs(), defsAndOptions.indexOptions())
                .stream();
    }
}

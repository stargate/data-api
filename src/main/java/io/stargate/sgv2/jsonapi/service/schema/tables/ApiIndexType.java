...
public enum ApiIndexType {
  ...
  static ApiIndexType fromCql(
    ...
    // If there is no function on the indexTarget, and the column is a scalar,
    // then it is a regular index on an int, text, etc.
    ...
    // then we will let building the index check the options.
    ...
    // then we will let building the collection index to check if the function is supported.

...
public class AlterTypeExceptionHandler extends KeyspaceDriverExceptionHandler {
  ...
  public AlterTypeExceptionHandler(
    ...
    this.allFieldRenames = allFieldRenames == null
        ? List.of()
        : List.copyOf(allFieldRenames);
    this.allAddFieldNames = allAddFieldNames == null
        ? List.of()
        : List.copyOf(allAddFieldNames);
  }
  ...
  public RuntimeException handle(InvalidQueryException exception) {
    ...
    if (exception.getMessage().contains("Unkown field")) {
      return SchemaException.Code.CANNOT_RENAME_UNKNOWN_TYPE_FIELD.get(
          "typeName", errFmt(udtName), "unknownField", getFieldName(exception.getMessage(),
              "Unkown field ", " in user type", allFieldRenames));
    }
    ...
    if (exception.getMessage().contains("a field with name")) {
      return SchemaException.Code.CANNOT_ADD_EXISTING_FIELD.get(
          Map.of("typeName", errFmt(udtName), "existingField",getFieldName(exception.getMessage(),
              "a field with name", "already exists", allAddFieldNames)));
    }
    ...
    if (exception.getMessage().contains("Duplicate field name")) {
      return SchemaException.Code.CANNOT_ADD_EXISTING_FIELD.get(
          Map.of("typeName", errFmt(udtName), "existingField",getFieldName(exception.getMessage(),
              "Duplicate field name", "in type", allAddFieldNames)));
    }

    return super.handle(exception);
  }

  private String getFieldName(String message, String prefix, String suffix, List<String> fallbacks) {

...
public abstract class BaseTask<
  ...
  // error
  //  // handling
  private ResultSupplierT resultSupplier;
   ...
   */
  protected BaseTask(int position, SchemaT schemaObject, RetryPolicy retryPolicy) {
  ...
  public <SubT extends Task<SchemaT>> Uni<SubT> execute(CommandContext<SchemaT> commandContext) {
    ...
    return Uni.createFrom()
        ...
        .onItemOrFailure()
        .transform((result, throwable) -> (SubT)onCompletion(result, throwable)) // HACK TODO
        .invoke(
            () -> {
              if (LOGGER.isDebugEnabled()) {
   ...
   */
  protected abstract ResultSupplierT buildResultSupplier(
   ...
   */
  protected <T extends Task<SchemaT>> T onCompletion(ResultT result, Throwable throwable) {
    ...
    var handledException =
        throwable instanceof RuntimeException re ? maybeHandleException(resultSupplier, re) : throwable;
   ...
   * TaskStatus#COMPLETED} themselves.
   *
   * @param resultSet The result set from the driver, this is the result of the query.
   * @return this object, cast to {@link SubT} for chaining methods.
   */
  protected <T extends Task<SchemaT>> T onSuccess(ResultT result) {
  ...
  protected <T extends Task<SchemaT>> T setStatus(TaskStatus newStatus) {
   ...
   * @return this object, cast to {@link SubT} for chaining methods.
   */
  public <T extends Task<SchemaT>> T checkTerminal(String context) {
  ...
  protected boolean swapStatus(
      String context, TaskStatus expectedStatus, TaskStatus newStatus) {
   ...
   * the attempt status to {@link TaskStatus#ERROR} if no non-null failure is added.
   *

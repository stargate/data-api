...
public record DeleteOperationPage(
  ...
  public CommandResult get() {
    ...
    if (deletedInformation == null) {
      ...
      if (returnDocument()) {
        if (singleDocument()) {
          ...
          return CommandResult.multiDocumentBuilder(false, false, null)
              .addStatus(CommandStatus.DELETED_COUNT, 0)
              .addDocument(null)
              .build();
        }
    ...
    if (returnDocument()) {
      if (singleDocument()) {
        builder = CommandResult.singleDocumentBuilder(false, false, null);
      } else {
        builder = CommandResult.multiDocumentBuilder(false, false, null);
      }
      ...
      builder = CommandResult.statusOnlyBuilder(false, false, null);
    }

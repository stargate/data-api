...
public abstract class BaseTask<
   ...
   * fragile.
   *
   * @param resultSupplier The supplier instances that created the successful results, this can be
   *                       used to smuggle extra information if needed.
   * @param result The {@link ResultT} generated by the task, e.g. an {@link AsyncResultSet}.
   */
  protected void onSuccess(ResultSupplierT resultSupplier, ResultT result) {
  ...
  protected void onCompletion(ResultT result, Throwable throwable) {
    ...
    switch (status()) {
      case IN_PROGRESS -> {
        ...
        if (result != null) {
          onSuccess(resultSupplier, result);
        } else {
  ...
  public Recordable.DataRecorder recordTo(Recordable.DataRecorder dataRecorder) {
    return dataRecorder
        ...
        .append("schemaObject.identifier", schemaObject.identifier())

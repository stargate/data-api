package io.stargate.sgv2.jsonapi.service.provider;

import com.fasterxml.jackson.annotation.JsonValue;

/**
 * The set of billing event types emitted by {@link Billing}.
 *
 * <p>Each event represents a single billable dimension of a model call. The {@code internal_*}
 * variants are emitted for providers configured in {@link
 * io.stargate.sgv2.jsonapi.config.BillingConfig#internalModelProviders()}; everything else is
 * {@code external_*}.
 *
 * <ul>
 *   <li>{@link #INTERNAL_MODEL_TOTAL_TOKENS} / {@link #EXTERNAL_MODEL_TOTAL_TOKENS} — total tokens
 *       reported by the model.
 *   <li>{@link #INTERNAL_MODEL_EGRESS_BYTES} / {@link #EXTERNAL_MODEL_EGRESS_BYTES} — bytes sent
 *       from the data plane to the model (request payload).
 *   <li>{@link #INTERNAL_MODEL_INGRESS_BYTES} / {@link #EXTERNAL_MODEL_INGRESS_BYTES} — bytes
 *       received from the model back to the data plane (response payload).
 * </ul>
 */
public enum BillingEventType {
  INTERNAL_MODEL_TOTAL_TOKENS("internal_model_total_tokens", true, Dimension.TOTAL_TOKENS),
  EXTERNAL_MODEL_TOTAL_TOKENS("external_model_total_tokens", false, Dimension.TOTAL_TOKENS),
  INTERNAL_MODEL_EGRESS_BYTES("internal_model_egress_bytes", true, Dimension.EGRESS_BYTES),
  EXTERNAL_MODEL_EGRESS_BYTES("external_model_egress_bytes", false, Dimension.EGRESS_BYTES),
  INTERNAL_MODEL_INGRESS_BYTES("internal_model_ingress_bytes", true, Dimension.INGRESS_BYTES),
  EXTERNAL_MODEL_INGRESS_BYTES("external_model_ingress_bytes", false, Dimension.INGRESS_BYTES);

  /** The billable dimension a {@link BillingEventType} measures. */
  public enum Dimension {
    TOTAL_TOKENS,
    EGRESS_BYTES,
    INGRESS_BYTES
  }

  private final String eventName;
  private final boolean internal;
  private final Dimension dimension;

  BillingEventType(String eventName, boolean internal, Dimension dimension) {
    this.eventName = eventName;
    this.internal = internal;
    this.dimension = dimension;
  }

  /** Lower-case event_type string used in the JSON billing event. */
  @JsonValue
  public String eventName() {
    return eventName;
  }

  public Dimension dimension() {
    return dimension;
  }

  /**
   * Resolves the event type for a given dimension and provider classification.
   *
   * @param dimension which billable dimension we are emitting
   * @param internal {@code true} if the model provider is configured as internal
   */
  public static BillingEventType of(Dimension dimension, boolean internal) {
    return switch (dimension) {
      case TOTAL_TOKENS -> internal ? INTERNAL_MODEL_TOTAL_TOKENS : EXTERNAL_MODEL_TOTAL_TOKENS;
      case EGRESS_BYTES -> internal ? INTERNAL_MODEL_EGRESS_BYTES : EXTERNAL_MODEL_EGRESS_BYTES;
      case INGRESS_BYTES -> internal ? INTERNAL_MODEL_INGRESS_BYTES : EXTERNAL_MODEL_INGRESS_BYTES;
    };
  }
}

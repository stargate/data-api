 ...
 *
 * <p><b>NOTE:</b> we do not use this in production, where we use the driver schema builder, this is
 * for testing. See {@link SuperShreddingBuilder} for the testing process.
 *
 * <p>The tempalates use the {@link StringSubstitutor} and in particular use the idea of a default
 * if the key is not present. <code>${VECTOR_COLUMN:-}</code> is an example, if not present an empty
...
public interface SuperShreddingCQL {
  ...
  interface IndexCQLAndDefs {
    ...
    List<IndexCQLAndDef> ALL_INDEXES =
        List.of(
            INDEX_EXIST_KEYS,
            INDEX_ARRAY_SIZE,
    ...
    List<IndexCQLAndDef> OPTIONAL_INDEXES =
    ...
    Map<IndexDef, IndexCQLAndDef> ALL_INDEXES_BY_INDEX_DEF =
        ALL_INDEXES.stream()

...
public class DefaultDriverExceptionHandler<SchemaT extends SchemaObject>
  ...
  // NOTE: to subclasses - the statement may be null, this happens when some Opertions work with metadata
  ...
  public RuntimeException handle(AllNodesFailedException exception) {
    ...
    return switch (highestPriority) {
      ...
      case RuntimeException re -> DatabaseException.Code.UNEXPECTED_DRIVER_ERROR.get(errVars(schemaObject, re));
  ...
  public RuntimeException handle(NoNodeAvailableException exception) {
    ...
    return DatabaseException.Code.FAILED_TO_CONNECT_TO_DATABASE.get(errVars(schemaObject, exception));
  }
  ...
  public RuntimeException handle(InvalidQueryException exception) {
    return DatabaseException.Code.INVALID_DATABASE_QUERY.get(errVars(schemaObject, exception,
        map -> {
          map.put("cql", CqlPrintUtil.trimmedCql(statement));
          map.put("values", errFmtJoin(CqlPrintUtil.trimmedPositionalValues(statement), Object::toString));
        }));
  }
  ...
  public RuntimeException handle(SyntaxError exception) {
    ...
    return DatabaseException.Code.UNSUPPORTED_DATABASE_QUERY.get(errVars(schemaObject, exception,
        map -> {
          map.put("cql", CqlPrintUtil.trimmedCql(statement));
          map.put("values", errFmtJoin(CqlPrintUtil.trimmedPositionalValues(statement), Object::toString));
        }));
  ...
  //    - Should not happen.
  // - **OverloadedException**
  //    - Reading C* code this happens when either number of hints or when the ReplicaFilteringProtection is working
  //      to repair inconsistency when queries use ALLOW FILTERING and the data is not consistent. While we use
  //      ALLOW FILTERING, this is an overload protection so will wait to see it happen in the wild before handling it.
  // - **QueryConsistencyException**
  //    - This is a parent for things like ReadFailureException, ReadTimeoutException, handle in those.

  /**
   * Thrown when the LWT proposal was not accepted by a quorum of nodes, so the LWT transaction is in an unknown state.
   * <p>
   * We should retry on this error in {@link io.stargate.sgv2.jsonapi.service.cqldriver.CqlProxyRetryPolicy} and when
   * we get to processing it here it means we could not make the LWT work.
   * @param exception
   * @return
   */
  @Override
  public RuntimeException handle(CASWriteUnknownException exception) {
    return DatabaseException.Code.FAILED_COMPARE_AND_SET.get(errVars(schemaObject, exception,
        map -> {
          map.put("cql", CqlPrintUtil.trimmedCql(statement));
          map.put("values", errFmtJoin(CqlPrintUtil.trimmedPositionalValues(statement), Object::toString));
        }));
  ...
  /**
   * Thrown when either UnavailableException | TimeoutException | InvalidRequestException happens in C*.
   * <p>
   * This should retry in the {@link io.stargate.sgv2.jsonapi.service.cqldriver.CqlProxyRetryPolicy} and if we get here
   * we assume there is a proper failure.
   * </p>
   * @param exception
   * @return
   */
  ...
  public RuntimeException handle(TruncateException exception) {
    ...
    return DatabaseException.Code.FAILED_TRUNCATION.get(errVars(schemaObject, exception,
        map -> {
          map.put("cql", CqlPrintUtil.trimmedCql(statement));
          map.put("values", errFmtJoin(CqlPrintUtil.trimmedPositionalValues(statement), Object::toString));
        }));
   ...
   * <p>
   *  This should be retried in the {@link io.stargate.sgv2.jsonapi.service.cqldriver.CqlProxyRetryPolicy} and if we get here
   *  then we know the database is really not available.
   * </p>
   * @param exception
   * @return
   */
   ...
   * <p>
   * Each node in the reason map needs to have a reason from the Cassandra class RequestFailureReason:
   * <ul>
   *   <li><code>UNKNOWN</code> (0)</li>
   *   <li><code>READ_TOO_MANY_TOMBSTONES</code> (1)</li>
   *   <li><code>TIMEOUT</code> (2)</li>
   *   <li><code>INCOMPATIBLE_SCHEMA</code> (3)</li>
   *   <li><code>INDEX_NOT_AVAILABLE</code> (4)</li>
   *   <li><code>UNKNOWN_COLUMN</code> (5)</li>
   *   <li><code>UNKNOWN_TABLE</code> (6)</li>
   *   <li><code>REMOTE_STORAGE_FAILURE</code> (7)</li>
   * </ul>
   */
  @Override
  public RuntimeException handle(ReadFailureException exception) {
    var uniqueReasons = exception.getReasonMap().values().stream()
        .distinct()
    ...
    return DatabaseException.Code.FAILED_READ_REQUEST.get(
        errVars(
            ...
            m -> {
              ...
              m.put("values", errFmtJoin(CqlPrintUtil.trimmedPositionalValues(statement), Object::toString));
  ...
  @FunctionalInterface
  public
  interface Factory<T extends SchemaObject> extends BiFunction<T, SimpleStatement, DriverExceptionHandler> {

    /**
     * Returns a factory that will call
     * {@link FactoryWithIdentifier#apply(SchemaObject, SimpleStatement, CqlIdentifier)} with the supplied identifier.
     */
    static <StaticT extends SchemaObject> Factory<StaticT> withIdentifier(FactoryWithIdentifier<StaticT> factoryWithIdentifier, CqlIdentifier identifier){
      return (schemaObject, statement) -> factoryWithIdentifier.apply(schemaObject, statement, identifier);
    }
  }

  /**
   * Most subclasses for command specific exception handlers will need to know the identifier of the object they are
   * working with. This interface provides function currying to allow the identifier to be passed and returns
   * the above regular {@link Factory} function.
   * <p>
   * Declare the constructor for the handler the same as
   * {@link FactoryWithIdentifier#apply(SchemaObject, SimpleStatement, CqlIdentifier)}, then pass it and the identifier
   * to {@link Factory#withIdentifier(FactoryWithIdentifier, CqlIdentifier)} to get a standard factory.
   * <p>
   * See usage of {@link io.stargate.sgv2.jsonapi.service.operation.tables.CreateIndexExceptionHandler}
   * @param <T> Type of the schema object the handler is for.
   */
  @FunctionalInterface
  public
  interface FactoryWithIdentifier<T extends SchemaObject> {

    /**
     * Signature of the constructor for the handler.
     */
    DriverExceptionHandler apply(T schemaObject, SimpleStatement statement, CqlIdentifier identifier);
  }
}

...
/**
 * General pattern for defining the properties of a super-shredding "table" and then building objects from that.
 *
 * <p>
 * Building these objects is tied up with how we create the statements to build a table, how we build a
 * predicate to test for a table, and how we build test data. Without repeating the table cql too  many
 * times and creating fragile tests that depend on cql strings.
 * See the test class <code>SuperShreddingBuilderTest</code>
 * </p>
 *
 * <p>
 * From the logical representation on this builder, we can create:
 * <ul>
 *     <li><code>cql</code> for testing (below) via {@link #cql()}</li>
 *     <li>{@link TableMetadata} and {@link com.datastax.oss.driver.api.core.metadata.schema.IndexMetadata} for testing (below), via {@link #metadata()}</li>
 *     <li>{@link com.datastax.oss.driver.api.core.cql.SimpleStatement} for creating a table at run time via TODO</li>
 *     <li>{@link SuperShreddingTablePredicate} for runtime testing if TableMetadata represents a super shredding table via {@link #predicate()}</li>
 * </ul>
 * </p>
 *
 *  <p>
 ...
 *  </p>
 * @param <T> Type of the object that represents the Super Shredding Component, such as string for cql
...
public abstract class SuperShreddingBuilder<T, U extends SuperShreddingBuilder<T, U>> {
    ...
    /**
     * Get a new {@link SuperShreddingPredicateBuilder} that can be used to build a {@link SuperShreddingTablePredicate}
     */
    ...
    /**
     * Implementations must implement and create all the components needed for the super shredding table.
     */
    ...
    /**
     * Builds all the components for the table, and returns only the value of the (first)
     * Table component. Use this to quickly get just the (say) "create table" cql.
     */
    ...
    /**
     * Builds all the components for this super shredding table, the table and the indexes
    ...
    /**
     * Holds a component of a super shredding table, such as the table or index. These are created by the
     * {@link SuperShreddingBuilder} implementations.
     *
     * @param identifier the name, table name or index name.
     * @param type the type of component, either table or index
     * @param value the value of the component, such as the table definition or index definition, or string
    ...
    /**
     * Holds all the index definitions and options for the super shredding table.
     * See {@link #indexDefsAndOptions(SuperShreddingDef)}
     *
     * @param indexDefs All indexes the super shredding table will have.
     * @param indexOptions All options for the indexes the super shredding table will have, keyed on the
     *                     indexDef. Not all indexes have options.
     */
    protected record IndexDefsAndOptions(List<IndexDef> indexDefs,
                                        Map<IndexDef, Map<String, String>> indexOptions){
    ...
    /**
     * Gets the index definitions and options for the super shredding table based on {@link SuperShreddingDef}
     * <p>
    ...
    protected IndexDefsAndOptions indexDefsAndOptions(SuperShreddingDef superShreddingDef){
        ...
        var indexDefs =  superShreddingDef.hasAnyOptional() ?
        ...
        if (superShreddingDef.isVectorDefined()) {
            ...
            IndexDef.vectorIndexOptions(superShreddingDef.similarityFunction(), superShreddingDef.sourceModel())

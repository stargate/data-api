...
public class UpdateTargetLocator {
  ...
  /**
   * Factory method for constructing path; also does minimal verification of
   * path: currently only verification is to ensure there are no empty segments.
   *
  ...
  /**
   * Method that will create target instance for configured path through given document;
   * if no such path exists, will not attempt to create path (nor report any problems)
   * but simply return {@link UpdateTarget} with specific information that is available
   * regarding path.
   *<p>
   * Resulting {@link UpdateTarget} will
   *</p>
   *
  ...
  /**
   * Method that will create target instance using configured path through given document; if no such path

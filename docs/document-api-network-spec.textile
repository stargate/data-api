...
* *Endpoints:* The URL address for clients to send request messages. There are two types of endpoints which accept different messages: database and collection endpoints. 
...
* Database Endpoint: used to send commands which apply to the whole Database, such as listing all the Collections. 
...
h3(#database-endpoint). Database Endpoint

The Database Endpoint has the form:

p. _Syntax:_

bc(syntax).. 
<database-endpoint> ::= <server-address>/<database-name>

<server-address> ::= // Server URI using HTTP(S), scheme://host/path 
<database-name> :: // TODO Rules for database names

p. _Sample:_

bc(sample).. 
https://stargate.mycompany.com/my-database

h4(#database-endpoint-errors). Errors

# Requests sent to a Database that was not previously created via administration tools results in a #TODO soft error. 
...
bc(syntax).. 
<collection-endpoint> ::= <server-address>/<database-name>/<collection-name>

<collection-name> :: // TODO rules for collection names

p. _Sample:_

bc(sample).. 
https://stargate.mycompany.com/my-database/users

p. Requests sent to a Collection that does not exist (using a valid database name) result in the creation of the Collection and delivery of the message. This may take longer than delivering a message to an existing Collection.
